...
### 1.2 เปิดพอร์ต 22 (HTTP)

อนุญาตการเชื่อมต่อผ่านพอร์ต **22**:

```bash
sudo ufw allow 22
```

### 1.3 เปิดพอร์ต 80 (HTTP)
...
2. กำหนดค่าต่อไปนี้:
    ...
    - **Protocols and ports**: เลือก **Specified protocols and ports** และเพิ่ม:
        - **tcp:22** (สำหรับ HTTP). // Default 
        - **tcp:80** (สำหรับ HTTP).
        - **tcp:8080** (สำหรับ HTTP).
